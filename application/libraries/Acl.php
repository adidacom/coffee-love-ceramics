...
class acl{
    ...
    public function hasPermission($ctrl="", $ssctrl=""){
        $allow = true;

        $pathToTest = $this->genPath($ctrl,$ssctrl);
        $user_group_id = $this->ci->ion_auth->get_users_groups()->row()->id;

        $action = $this->ci->action_model->openWhere(array('path'=>$pathToTest));
        if(count($action) == 0)
            return false;

        $exist = $this->ci->group_action_model->count(array('id_group'=>$user_group_id, 'id_action'=>$action[0]->id));
