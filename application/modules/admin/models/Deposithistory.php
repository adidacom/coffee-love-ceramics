...
class DepositHistory extends MY_Model {

    protected $table_name = 'tbl_deposit_history';
    ...
    public function get_sum_groupby_paymenttypes_by_bank($bank_id){
        $query = $this->db->select("id,  type_name, (SELECT IFNULL(SUM(withdraw_amount),0) FROM tbl_deposit_history WHERE payment_type_id = mpt.id AND bank_id=".$bank_id." ) AS ordered_sum")
            ->where("kind",'deposit')
